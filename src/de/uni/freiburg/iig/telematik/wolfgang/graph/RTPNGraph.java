...
public class RTPNGraph extends PNGraph {

	public RTPNGraph(GraphicalTimedNet netContainer, RTPNProperties properties) throws ParameterException {
	...
	protected String getArcConstraint(AbstractFlowRelation relation) {
		return String.valueOf(((AbstractPTFlowRelation) relation).getWeight());
	}
...
@Override
	...
	public AbstractMarking inOrDecrementPlaceState(PNGraphCell cell, int wheelRotation) throws ParameterException {
		TimedMarking initialMarking = getNetContainer().getPetriNet().getInitialMarking();
		TimedMarking oldValue = initialMarking;
	...
	protected String getPlaceToolTip(PNGraphCell cell) {
		TimedNetPlace ptPlace = getNetContainer().getPetriNet().getPlace(cell.getId());
	...
	public void updatePlaceCapacity(String name, String color, int newCapacity) {
		TimedNetPlace place = getNetContainer().getPetriNet().getPlace(name);
		if (newCapacity <= 0)
